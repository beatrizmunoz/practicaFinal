	...
	private class Counter extends Handler {
		...
		public void handleMessage(Message msg) {
			if (Iservice != null){
				...
				long timeFirstPart = maximo-drivingDate.getTime();
				Date periodo = new Date(0);
				
				switch (type){
				case 1 : 
					int timeSecondPart = (int)(actual.getTime() - timeDriving.getTimeInit().getTime());
					Integer[] tiempo = DesglossedTime.getTimers(timeSecondPart);
					periodo.setSeconds(tiempo[0]);
					periodo.setMinutes(tiempo[1]);
					periodo.setHours(tiempo[2]);
					myDate = DesglossedTime.getStringTime(timeFirstPart-periodo.getTime());
					break;
				case 0 :
					myDate = " ";
					break;
				default :
					myDate = DesglossedTime.getStringTime(timeFirstPart);
				}			

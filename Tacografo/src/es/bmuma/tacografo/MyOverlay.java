...
public class MyOverlay extends Overlay{
	...
	// Make sure to give mMarker bounds so it will draw in the overlay
			...
			p.toPixels(mGeoPoint, secondPoint);
 
			canvas.drawText(text, secondPoint.x - intrinsicWidth , secondPoint.y + intrinsicHeight, paint);
			super.draw(canvas, mapView, shadow);
			drawAt(canvas, mMarker, secondPoint.x + mMarkerXOffset, secondPoint.y + mMarkerYOffset, shadow);
		}
	}
}
